...
class VedaContext:
    def __init__(self, assistant_ref):
        ...
        # Initialize with current time to prevent immediate alert on startup
        self.last_entropy_alert = time.time()
    ...
    def _monitor_loop(self):
        if not gw:
            print("Context Monitor: Unsupported platform (No pygetwindow).")
            return
        ...
        while self.is_monitoring:
            try:
                ...
                # Zero-Trust Entropy Management: High threshold, long cooldown, HUD-only.
                # Threshold increased to 80, Cooldown to 4 hours (14400s)
                if visible_count > 80 and (time.time() - self.last_entropy_alert > 14400):
                    self.assistant.gui.after(0, lambda: self.assistant.gui.update_tactical_status("ENTROPY CRITICAL: Suggest Focus Mode"))
                    self.last_entropy_alert = time.time()
